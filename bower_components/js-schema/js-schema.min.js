(function e(t, n, r) {
	...
	1: [function(require, module, exports) {
		...
		"./lib/extensions/Array": 3,
		"./lib/extensions/Boolean": 4,
		"./lib/extensions/Function": 5,
		"./lib/extensions/Number": 6,
		"./lib/extensions/Object": 7,
		"./lib/extensions/Schema": 8,
		"./lib/extensions/String": 9,
		"./lib/patterns/anything": 10,
		"./lib/patterns/class": 11,
		"./lib/patterns/equality": 12,
		"./lib/patterns/nothing": 13,
		"./lib/patterns/object": 14,
		"./lib/patterns/or": 15,
		"./lib/patterns/reference": 16,
		"./lib/patterns/regexp": 17,
		"./lib/patterns/schema": 18,
		"./lib/schema": 19
	}],
	2: [function(require, module, exports) {
		...
		Schema.prototype = {
			...
			toJSON: session(function(makeReference) {
				...
				if (!session.serialized) session.serialized = {
					objects: [],
					jsons: [],
					ids: []
				};
				...
				if (makeReference && index !== -1) {
					...
					if (json.id == null) {
						do {
							json.id = "id-" + Math.floor(Math.random() * 1e5)
	...
	3: [function(require, module, exports) {
		...
		var ArraySchema = module.exports = Schema.extensions.ArraySchema = Schema.extend({
			...
			errors: function(instance) {
				var self = this;
				if (!(instance instanceof Array)) return instance + " is not an instance of Array";
				if (this.min === this.max) {
					if (instance.length !== this.min) return "Array length should be equal to " + this.min + " and is " + instance.length
				} else {
					if (this.min > 0 && instance.length < this.min) return "Array length should not be less than " + this.min + " and is " + instance.length;
					if (this.max < Infinity && instance.length > this.max) return "Array length should not be more than " + this.max + " and is " + instance.length
				}
				var results = {};
		...
		"../BaseSchema": 2,
		"../patterns/anything": 10,
		"../patterns/equality": 12
	}],
	...
	6: [function(require, module, exports) {
		...
		var NumberSchema = module.exports = Schema.extensions.NumberSchema = Schema.extend({
			...
			validate: function(instance) {
				return Object(instance) instanceof Number && (this.exclusiveMinimum ? instance > this.minimum : instance >= this.minimum) && (this.exclusiveMaximum ? instance < this.maximum : instance <= this.maximum) && (this.divisibleBy === 0 || instance % this.divisibleBy === 0)
			},
			toJSON: function() {
	...
	8: [function(require, module, exports) {
		...
		Schema.fromJSON.def(function(sch) {
			...
			if (!session.deserialized) session.deserialized = {
				references: {},
				subscribers: {}
			};
			if (sch.id != null) {
		...
		var ObjectSchema = module.exports = Schema.patterns.ObjectSchema = Schema.extend({
			...
			validate: function(instance) {
				...
				var stringPropsValid = Object.keys(this.stringProps).every(function(key) {
					return self.stringProps[key].min === 0 && !(key in instance) || self.stringProps[key].value.validate(instance[key])
				});
				...
				for (var key in instance) {
					...
					var regexpPropsValid = Object.keys(this.regexpProps).every(function(key) {
						return !self.regexpProps[key].key.test(key) || (checked = true) && self.regexpProps[key].value.validate(instance[key])
					});
